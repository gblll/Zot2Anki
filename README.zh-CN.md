...
**v0.2.0-rc.4：Windows 预发布版。** 将一篇 Zotero 生词 Note 同步到 Anki，并保留卡片身份、个人 Notes 和复习历史。仓库已公开；这不是稳定版。
...
验证基线为 **Windows x64、Python 3.13、Anki 26、PyMuPDF 1.28.2**。请先安装 Python 和 Anki；发行包不包含这些运行时。环境不匹配时，会在打开个人数据库前退出。
...
GitHub Support 于 2026-09-07 确认清理未引用提交；2026-09-08 在已登录浏览器中核验，两个已知受影响的旧提交页面均显示 404。核验范围限于这两个页面，不代表检查了所有历史缓存。仓库已公开，rc.2、rc.3 和 rc.4 均已公开预发布。rc.4 支持 Anki 26，并增加更清晰的同步进度；发行附件和验收结果绑定准确的发行提交，已有发行、标签和附件保留不变。本轮仅使用合成数据，历史真实资料检查不视为 rc.4 验收。
