...
**v0.2.0-rc.4：Windows 预发布候选版。** 将一篇 Zotero 生词 Note 同步到 Anki，并保留卡片身份、个人 Notes 和复习历史。仓库已公开；本候选版准备为 Draft Pre-release，不是稳定版。

原文例句和底部来源显示“文献标题 · 期刊缩写 · p. 页码”。期刊优先取 Zotero 文献条目的 `journalAbbreviation`，为空时使用 `publicationTitle`，两者都为空则省略期刊。期刊使用斜体，不加括号；对可识别的缩略词补句点（如 `Nat Commun` → `Nat. Commun.`），完整单词和 `IEEE`、`ACS` 等首字母缩写保持原样。可在 Zotero 中维护这两个字段，后续同步会更新出处并保留复习历史。在线例句使用自身公开出处；本地期刊信息不会加入 clean 分享包。

## 安装

验证基线为 **Windows x64、Python 3.13、Anki 26.05 或 26.09、PyMuPDF 1.28.2**。请先安装 Python 和 Anki；发行包不包含这些运行时。环境不匹配时，会在打开个人数据库前退出。
...
GitHub Support 于 2026-09-07 确认清理未引用提交；2026-09-08 在已登录浏览器中核验，两个已知受影响的旧提交页面均显示 404。核验范围限于这两个页面，不代表检查了所有历史缓存。仓库已公开，rc.2 和 rc.3 分别于 2026-09-08、2026-09-13 公开预发布。rc.4 增加 Anki 26.09 支持和更清晰的同步进度，发行附件按准确提交构建和验收；已有发行、标签和附件保留不变。本轮仅使用合成数据，历史真实资料检查不视为 rc.4 验收。
